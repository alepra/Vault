...
let currentGame = null; // legacy (kept for existing endpoints)
let gameTimer = null;
...
async function addParticipant(name, isHuman = true, botType = 'standard') {
  ...
  return addParticipantToGame(currentGame, name, isHuman, botType);
}

function addParticipantToGame(game, name, isHuman = true, botType = 'standard', personality = null) {
  if (!game) return null;
  const participant = {
    id: generateId(),
    name,
...
async function startGameForSession(sessionId) {
  ...
  if (!sessionIPO[sessionId]) {
    sessionIPO[sessionId] = new IPOModule(gameStates[sessionId], io, sessionId, ledgerModule);
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', async (data) => {
    ...
    const participant = addParticipantToGame(socket.gameState, data.name, true);
    if (participant) {
  ...
  socket.on('submitIPOBids', async (bidData) => {
    ...
    if (socket.gameState && socket.gameState.phase === 'ipo') {
      ...
      setTimeout(() => {
        ...
        if (gameStates[socket.sessionId] && gameStates[socket.sessionId].phase === 'newspaper') {
          socket.emit('ipoCompleted', { message: 'IPO completed - moving to newspaper phase!' });
        } else {
  ...
  socket.on('skipIPO', async () => {
    ...
    if (socket.gameState && socket.gameState.phase === 'ipo') {
      ...
      if (!sessionIPO[socket.sessionId]) {
        sessionIPO[socket.sessionId] = new IPOModule(gameStates[socket.sessionId], io, socket.sessionId, ledgerModule);
      }
...
// Store game states by session ID (moved to top)
