...
const { BotProfile, BotProfiles } = require('./botProfile');

class IPOModule {
  constructor(gameState, io, sessionId = 'shared_game', ledgerModule = null, eventStoreIntegration = null) {
    ...
    try {
      console.log('🧩 IPOModule constructed. Ledger provided:', !!ledgerModule);
      console.log('🧩 Ledger methods available:', {
        initializeParticipant: typeof this.ledger.initializeParticipant,
        recordPurchase: typeof this.ledger.recordPurchase,
        getAllLedgers: typeof this.ledger.getAllLedgers
      });
    } catch (e) { /* ignore logging errors */ }
    this.eventStoreIntegration = eventStoreIntegration;
    this.botNames = new BotNameModule();
    
    // Assign bot profiles to AI participants
    this.assignBotProfiles();
  }

  /**
   * Assign bot profiles to AI participants based on their personalities
   */
  assignBotProfiles() {
    if (!this.gameState || !this.gameState.participants) return;
    
    const profileTypes = Object.keys(BotProfiles);
    let profileIndex = 0;
    
    for (const participant of this.gameState.participants) {
      if (!participant.isHuman) {
        // Assign profile based on personality or round-robin
        if (participant.personality && participant.personality.bidStrategy) {
          const strategy = participant.personality.bidStrategy;
          if (strategy === 'scavenger') {
            participant.botProfile = BotProfiles.scavenger;
          } else if (strategy === 'ceo') {
            participant.botProfile = BotProfiles.ceo;
          } else if (strategy === 'high') {
            participant.botProfile = BotProfiles.aggressive;
          } else if (strategy === 'low') {
            participant.botProfile = BotProfiles.conservative;
          } else {
            participant.botProfile = BotProfiles.balanced;
          }
        } else {
          // Round-robin assignment
          participant.botProfile = BotProfiles[profileTypes[profileIndex % profileTypes.length]];
          profileIndex++;
        }
        
        console.log(`🤖 Assigned ${participant.botProfile.archetype} profile to ${participant.name}`);
      }
    }
  }
    ...
    for (const bot of scavengerBots) {
      ...
      if (sharesForThisBot > 0) {
        ...
        if (totalCost <= availableCash) {
          // Record purchase in ledger (with Event Store integration)
          let success;
  ...
  /**
   * Process all bids for a specific company using authoritative Dutch auction
   * ENHANCED: Proper guardrails, atomic allocation, structured logging
   */
  async processCompanyBids(company, bids) {
    try {
      ...
      // ATOMIC ALLOCATION - Single critical section
      ...
      for (const bid of bids) {
        ...
        let sharesToAllocate = Math.min(bid.shares, availableShares - sharesAllocated);
        
        if (sharesToAllocate > 0) {
          // Apply bot guardrails
          if (!participant.isHuman && participant.botProfile) {
            const currentShares = participant.shares?.[company.id] || 0;
            const newOwnership = (currentShares + sharesToAllocate) / company.shares;
            
            // Enforce ownership cap
            if (newOwnership > participant.botProfile.ownershipCapPct) {
              const maxAllowedShares = Math.floor(company.shares * participant.botProfile.ownershipCapPct) - currentShares;
              if (maxAllowedShares <= 0) {
                console.log(`🚫 ${participant.name} skipped - ownership cap reached (${(newOwnership * 100).toFixed(1)}%)`);
                continue;
              }
              sharesToAllocate = Math.min(sharesToAllocate, maxAllowedShares);
            }
          }
          ...
          if (success) {
            ...
            // Get current ledger state and sync
            const ledger = this.ledger.ledgers.get(participant.id);
            if (ledger) {
              participant.remainingCapital = ledger.cash;
              participant.totalSpent = 1000 - ledger.cash;
            }
            ...
            // Record allocation result for structured logging
            allocationResults.push({
              ...
              ownershipPct: (sharesToAllocate / company.shares) * 100
            });
            
            console.log(`💰 ${participant.name}: ${sharesToAllocate} shares at $${clearingPrice.toFixed(2)} (bid: $${bid.price.toFixed(2)}) - ${(sharesToAllocate / company.shares * 100).toFixed(1)}% ownership`);
          } else {
            console.error(`❌ Failed to record IPO purchase for ${participant.name}`);
          }
        }
      }
      ...
      if (sharesAllocated !== availableShares) {
        ...
        console.log(`✅ ${company.name}: All ${availableShares} shares sold at $${clearingPrice.toFixed(2)}`);
      }

      // Store the clearing price and create IPO result
      company.ipoPrice = clearingPrice;
      company.currentPrice = clearingPrice;
      company.lastPrice = clearingPrice;

      // Create structured IPO result for newspaper
      const ipoResult = {
      ...
      // Store IPO result for newspaper phase
      if (!this.gameState.ipoResults) this.gameState.ipoResults = [];
      this.gameState.ipoResults.push(ipoResult);

      // Determine CEO
      this.determineCEO(company);
      
      console.log(`📰 IPO result stored for ${company.name}: $${clearingPrice.toFixed(2)} clearing price, ${allocationResults.length} allocations`);
  ...
  /**
   * Store and immediately process human bids, then trigger AI bidding
   * Called when human submits bids
   */
  async storeHumanBids(bids, participantId) {
    try {
      console.log('📝 Processing human bids for participant:', participantId);
      console.log('📝 Number of bids:', bids.length);
      
      // Store bids for processing
      this.humanBids = bids;
      this.humanParticipantId = participantId;
      
      // Immediately process human bids
      if (bids && bids.length > 0 && participantId) {
        console.log('👤 Processing human bids asynchronously');
        // Kick off async processing without blocking the caller (ack can return immediately)
        this.processHumanBids(bids, participantId)
          .then(() => {
            console.log('✅ Human bids processed (async), AI bidding handled');
          })
          .catch((error) => {
            console.error('❌ Error processing human bids (async):', error);
          });
      } else {
        console.log('⚠️ No valid bids to process, starting AI bidding directly (async)');
        this.processAIBids()
          .then(() => {
            console.log('✅ AI bids processed (async)');
          })
          .catch((error) => {
            console.error('❌ Error processing AI bids (async):', error);
          });
      }
   ...
   * ENHANCED: Proper phase transition with IPO results
   */
  completeIPO() {
    ...
    // Validate all companies have IPO results
    const ipoResults = this.gameState.ipoResults || [];
    console.log(`📰 IPO Results Summary: ${ipoResults.length} companies processed`);
    
    this.gameState.companies.forEach(company => {
      if (company.ipoPrice) {
        console.log(`📊 ${company.name} final IPO price: $${company.ipoPrice.toFixed(2)}, shares sold: ${company.totalSharesAllocated || 0}`);
      } else {
        console.error(`❌ ${company.name} has no IPO price set - this should not happen`);
      }
    });
    
    // Set phase to newspaper and emit update
    this.gameState.phase = 'newspaper';
    this.gameState.ipoCompletedAt = new Date().toISOString();
    
    // Emit game state update to all clients in this session
    this.io.to(this.sessionId).emit('gameStateUpdate', this.gameState);
    this.io.to(this.sessionId).emit('ipoCompleted', { 
      message: 'IPO completed successfully', 
      ...
      if (this.ledger && this.ledger.ledgers) {
        for (const [participantId, ledgerData] of this.ledger.ledgers) {
          ...
          if (ledgerData.stockPositions) {
            for (const [companyId, positions] of ledgerData.stockPositions) {
              // positions is an array of purchase lots, not a single position
